...
> 最近更新：2026-05-30（校正：R4-2 / R1 已完成，移入归档）
...
| ID | 问题 | 修复方向 | 文件 |
|---|---|---|---|
| **i18n 长尾** | 深层插值标签未双语：实体详情统计、FloatingIcon tooltip、Onboarding 引导、设置页 隐私/关于 section | 同 P1 模式：加 key + useTranslation | 实体详情等 |
...
**2026-05-22~24（已落 main）**
- R4-2 复制可撤销：回执 toast 加"撤销"按钮，5s 内恢复旧剪贴板（`811681f`）
- R1 TTS 首次朗读一键开启引导（`c0a9b5d`）
- i18n 长尾：回放详情 PipelineDetailView 深层标签双语（`dd5fe95`）
- E3 首迁：NeverDialog → `shared/Modal`（`b98a2ad`）
- E1/#3：抽出 `kg/drafts-store.ts`（草稿台+反向校准）（`ef49084`）
- 放弃 openclaw 后端（`7729559`）
...
> 详细修复历史见 git log 与 `docs/archive/history/REFLECTION_LOG.md`、`docs/archive/audits/REVIEW_REPORT.md`（点对点审计报告，保留作存档）。
