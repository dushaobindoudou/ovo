...
export function useAgentBridge() {
  ...
  const getStatus = useCallback(async () => {
    ...
    try { return await window.nudgeAPI.agent.status(); } catch { return null; }
  }, []);
  const setBackend = useCallback(async (backend: string) => {
    if (!isElectron) return;
    try { await window.nudgeAPI.agent.setBackend(backend); } catch { /* ignore */ }
  }, []);
  const setApiConfig = useCallback(async (config: AgentApiConfig) => {
    if (!isElectron) return { ok: false };
    try { return await window.nudgeAPI.agent.setApiConfig(config); } catch { return { ok: false }; }
  }, []);
  const testScenario = useCallback(async (scenarioId: string, customPrompt?: string) => {
    if (!isElectron) return null;
    try { return await window.nudgeAPI.agent.testScenario({ scenarioId, customPrompt }); } catch { return null; }
  }, []);
